            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5">
                <div>
                    <div className="inline-flex items-center gap-2 bg-white/5 px-3 py-1 rounded-full border border-white/10 w-fit mb-4">
                        ...
                        <span className="text-[8px] font-black text-white/50 uppercase tracking-[0.2em]">Location Directory</span>
                    </div>
                    <h1 className="text-3xl font-black text-white tracking-tighter uppercase leading-none mb-2">
                        System <span className="text-primary italic">Destinations</span>
                    </h1>
                    <p className="text-gray-500 font-bold uppercase tracking-[0.1em] text-[10px]">
                        Architecting the world's most immersive travel hubs
                    </p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-white/5 bg-white/[0.02]">
                                <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Destination Name</th>
                                <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Geographical Data</th>
                                <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest">Optimal Season</th>
                                <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-widest text-right">Admin Controls</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="px-8 py-20 text-center text-gray-600 font-bold uppercase tracking-widest text-xs">
                                        No destination clusters available in sector.
                                    </td>
                                </tr>
                            ) : (
                                destinations.map((dest) => (
                                    <tr key={dest.id} className="group hover:bg-white/[0.02] transition-colors">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className="relative w-12 h-12 shrink-0 group-hover:scale-110 transition-transform duration-500">
                                                    <img src={dest.image_url} alt="" className="w-full h-full rounded-xl object-cover grayscale-[0.3] group-hover:grayscale-0 transition-all" />
                                                    <div className="absolute inset-0 rounded-xl ring-1 ring-inset ring-white/10"></div>
                                                </div>
                                                <span className="font-black text-base text-white tracking-tight uppercase group-hover:text-primary transition-colors">{dest.name}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-2 text-gray-400">
                                                <Globe size={12} className="text-primary/60" />
                                                <span className="text-xs font-bold uppercase tracking-tight">{dest.city}, {dest.country}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-2 text-gray-500">
                                                <Calendar size={12} />
                                                <span className="text-[10px] font-black uppercase tracking-[0.1em]">{dest.best_time_to_visit || "N/A"}</span>
                                            </div>
                                        </td>
