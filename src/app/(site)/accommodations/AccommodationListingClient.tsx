...
import { Search, MapPin, Clock, Star, MoveRight, Loader2, Compass, Users, ArrowRight, X, ChevronDown, Check, Home } from "lucide-react";
...
function AccommodationListingContent() {
    const { page } = usePageContent('accommodations');
    ...
    return (
        <main className="bg-background min-h-screen pb-32">
            <ImmersiveHero
                title={page?.hero_title || "Luxury Accommodations"}
                subtitle={page?.hero_subtitle || "From high-altitude basecamps to luxury mountain resorts, find your home away from home."}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 relative z-20">
                ...
                <div className="text-center max-w-3xl mx-auto mb-12">
                    <h2 className="text-3xl md:text-4xl font-black text-white mb:6 uppercase tracking-tighter">Stay in <span className="text-gradient">Style & Comfort</span></h2>
                    <p className="text-gray-400 font-medium">
                        Elevate your mountain experience with our selection of handpicked accommodations. We ensure comfort even in the most remote corners of the North.
                    </p>
                </div>
                ...
                <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-[2rem] mb-16 flex flex-wrap items-center justify-between gap-6 shadow-2xl relative z-30">
                    <p className="text-gray-400 font-medium pl-4 border-l-2 border-primary">
                        Showing <span className="text-white font-bold">{accommodations.length}</span> luxury stays
                    </p>
                        ...
                        <div className="relative" ref={filterRef}>
                            ...
                            {showFilters && (
                                <div className="absolute top-full right-0 mt-2 w-72 bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl z-50 animate-fade-in-up">
                                    <h3 className="text-white font-bold mb-4 uppercase tracking-wider text-sm border-b border-white/10 pb-2">Filter Stays</h3>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <p className="text-gray-400 font-bold uppercase tracking-[0.3em] text-xs">Loading Stays...</p>
                    </div>
                ) : accommodations.length === 0 ? (
                    <div className="text-center py-32 bg-secondary/30 rounded-[3rem] border border-white/5">
                        <Home size={48} className="text-gray-600 mx-auto mb-6" />
                        <h3 className="text-3xl font-black text-white mb-4 uppercase">No Stays Found</h3>
                        <p className="text-gray-500 mb-8">Try adjusting your filters.</p>
                        <Link href="/accommodations" className="text-primary font-bold uppercase tracking-widest text-xs hover:underline">Reset Filters</Link>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {accommodations.map((stay, idx) => (
                            <Link key={stay.id} href={`/accommodations/${stay.id}`} className={`group relative rounded-[2.5rem] overflow-hidden shadow-2xl hover:shadow-orange/20 transition-all duration-700 ${idx % 3 === 1 ? 'lg:translate-y-12' : ''}`}>
                                <div className="aspect-[3/4] overflow-hidden bg-slate-800">
                                    <img
                                        src={stay.image_url || "/images/gallery-naran.jpg"}
                                    ...
                                    <div className="absolute top-6 left-6 flex flex-col gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                        <div className="bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20 inline-flex items-center gap-2">
                                            ...
                                            <span className="text-[9px] font-black text-white uppercase tracking-widest">{stay.destination?.name || "Pakistan"}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-10 transform translate-y-0 lg:translate-y-4 lg:group-hover:translate-y-0 transition-transform duration-500">
                                        ...
                                        <h3 className="text-3xl font-black text-white mb-4 tracking-tighter uppercase leading-none">{stay.title}</h3>
                                        ...
                                        <div className="flex flex-wrap gap-4 mb-8 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-700 delay-100">
                                            <div className="flex items-center gap-2 text-white/80">
                                                ...
                                                <span className="text-xs font-bold uppercase tracking-wider">Guests {stay.max_guests || 2}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-white/80">
                                                <Clock size={14} className="text-primary" />
                                                <span className="text-xs font-bold uppercase tracking-wider">{stay.total_nights} Nights</span>
                                            </div>
                                            {/* Price */}
                                            <div className="w-full pt-4 border-t border-white/20 flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-xl font-black text-white tracking-tighter">${stay.price}</p>
                                                </div>
...
export default function AccommodationListingClient() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-background flex items-center justify-center font-bold text-gray-700 uppercase tracking-[0.5em] text-xs">Loading Stays...</div>}>
