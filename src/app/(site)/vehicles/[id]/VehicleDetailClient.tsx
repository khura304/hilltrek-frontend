...
export default function VehicleDetailClient({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchReviews = async () => {
        try {
            const response = await api.get(`/reviews?vehicle_id=${resolvedParams.id}`);
    ...
    const handleBooking = async (e: React.FormEvent) => {
        ...
        if (!travelDate) {
            setBookingStatus({ type: 'error', message: "Selection Required: Select a travel date." });
            return;
        }
    ...
    if (error || !vehicle) {
        return (
            <main className="min-h-screen flex flex-col items-center justify-center bg-background text-center">
                <Car size={48} className="text-gray-700 mb-6" />
                <h1 className="text-3xl font-black text-white mb-4 uppercase">Vehicle Not Found</h1>
                <Link href="/vehicles" className="text-primary hover:text-primary-hover text-sm font-bold uppercase tracking-widest underline underline-offset-4">
                    Return to Vehicles
                </Link>
            </main>
        );
    ...
    return (
        <main className="flex-grow bg-background min-h-screen">
            <ImmersiveHero
                title={vehicle.title}
                subtitle={`Premium Transport in ${vehicle.destination?.name || 'Pakistan'}`}
                image={vehicle.image_url || "/images/hero-bg.jpg"}
                badge="Premium Fleet"
                stats={[
                    { label: "Duration", value: `${vehicle.duration_days} Days`, icon: <Clock size={20} /> },
                    { label: "Capacity", value: `${vehicle.max_passengers} Seats`, icon: <Users size={20} /> },
                    { label: "Type", value: "4x4 SUV", icon: <Car size={20} /> },
                    { label: "Condition", value: "Excellent", icon: <ShieldCheck size={20} /> }
                ]}
            />
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-8 space-y-16">
                        {/* Overview */}
                        <section>
                            <h2 className="text-3xl font-bold text-white mb-6 tracking-tight">Vehicle Description</h2>
                        ...
                        {/* Features Grid */}
                        <section>
                            <h2 className="text-3xl font-bold text-white mb-8 tracking-tight">Key Features</h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                )) : <p className="text-gray-500">Standard premium features apply.</p>}
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="bg-secondary/30 p-8 rounded-2xl border border-white/5">
                                <h3 className="text-lg font-bold text-white mb-6">Rate this Vehicle</h3>
                                <form onSubmit={handleReviewSubmit} className="space-y-6">
                                    ...
                                    <textarea
                                        className="w-full bg-background border border-white/10 rounded-xl p-4 text-white text-sm focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-gray-700"
                                        rows={3}
                                        placeholder="Tell others about the vehicle and driver..."
                                        value={reviewComment}
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        ...
                        <div className="bg-secondary border border-white/10 rounded-[4rem] p-12 shadow-2xl">
                            <div className="mb-8 pb-8 border-b border-white/5">
                                <span className="text-gray-500 text-xs font-bold uppercase tracking-widest block mb-2">Daily Rate From</span>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-4xl font-black text-white">${vehicle.price}</span>
                                    <span className="text-gray-500 text-xs font-bold">/ Day</span>
                                </div>
                            </div>

                            <form onSubmit={handleBooking} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block">Pickup Date</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block">Passengers</label>
                                    <div className="relative">
                                        <Users size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-primary" />
                                        <input
                                            type="number"
                                            min="1"
                                            max={vehicle.max_passengers || 10}
                                ...
                                <div className="space-y-4 pt-4 border-t border-white/5 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    <h4 className="text-xs font-bold text-white uppercase tracking-widest mb-4">Passenger Details</h4>
                                    {travelerDetails.map((traveler, idx) => (
                                        <div key={idx} className="space-y-4 p-6 bg-background/50 rounded-2xl border border-white/5">
                                            <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">Passenger #{idx + 1}</p>
                            ...
                            <div className="mt-6 flex items-center justify-center gap-2 text-gray-600">
                                <ShieldCheck size={14} />
                                <span className="text-[10px] uppercase font-bold tracking-widest">Secure Booking</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-secondary/30 rounded-3xl p-6 border border-white/5">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-500 text-xs">Vehicle support 24/7</p>
                                </div>
                            </div>
                            <div className="space-y-3">
                                <div className="flex items-center gap-3 text-sm text-gray-400 hover:text-white transition-colors cursor-pointer">
                                    <Mail size={14} /> <span>transit@hilltrek.com</span>
                                </div>
                                <div className="flex items-center gap-3 text-sm text-gray-400 hover:text-white transition-colors cursor-pointer">
                                    <Phone size={14} /> <span>+92 300 0000001</span>
                                </div>
                            </div>
                        </div>
                    </div>
