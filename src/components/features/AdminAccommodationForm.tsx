...
import api, { Destination, Accommodation } from "@/lib/api";
...
export default function AdminAccommodationForm({ accommodation, isEditing }: AdminAccommodationFormProps) {
    ...
    useEffect(() => {
        const fetchDestinations = async () => {
            try {
                ...
                if (!accommodation && response.data.length > 0) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast("error", err.response?.data?.message || "Failed to save accommodation.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-12 max-w-4xl bg-white/5 p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-white/5 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Accommodation Title</label>
                    <input
                        type="text"
                        required
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Rate per Stay ($)</label>
                    <input
                        type="number"
                        required
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Total Nights</label>
                    <input
                        type="number"
                        required
                        className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-4 text-white focus:ring-2 focus:ring-primary/50 outline-none transition-all font-medium"
                ...
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Guest Capacity</label>
                    <input
                        type="number"
                        required
                        className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-4 text-white focus:ring-2 focus:ring-primary/50 outline-none transition-all font-medium"
            ...
            <div className="space-y-2 relative z-10">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Stay Description</label>
                <textarea
                    required
                    className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-4 text-white focus:ring-2 focus:ring-primary/50 outline-none transition-all font-medium placeholder:text-white/20"
                    rows={5}
            ...
            <div className="flex items-center gap-4 py-4 relative z-10 bg-white/5 p-6 rounded-2xl border border-white/5">
                ...
                <label htmlFor="is_featured" className="text-[10px] font-black text-white uppercase tracking-widest cursor-pointer select-none">Prioritize as Featured Stay</label>
            </div>

            {/* Features Section */}
            <div className="space-y-6 relative z-10">
                <div className="flex justify-between items-center border-b border-white/5 pb-4">
                    <h3 className="text-sm font-black text-white uppercase tracking-widest px-2">Amenities & Features</h3>
                    <button
                        type="button"
                    ...
                    {formData.features.map((feat: string, idx: number) => (
                        <div key={idx} className="relative group">
                            <input
                                ...
                                placeholder="e.g. WiFi / 24h Hot Water"
                            />
                            <button
                                type="button"
            ...
            >
                {loading ? "Transmitting Data..." : isEditing ? "Update Accommodation" : "Deploy Accommodation Asset"}
            </button>
        </form>
    );
}
